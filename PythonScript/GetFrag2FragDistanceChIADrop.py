...
PairLoopOUT = open(sys.argv[1] + ".L2L.stat", "w")
...
while True:
    ...
    while i < FragCount:
        ...
        F2FFlag = 1
        j = i + 1
        while j < int((len(word) - CountColumn) / 3):
            NeighBorFrag = Fragment(word[j * 3 + CountColumn], word[j * 3 + CountColumn + 1], word[j * 3 + CountColumn + 2])
